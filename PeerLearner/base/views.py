...
def complete_task(request, pk):
    ...
    task.mark_complete()
    messages.success(request, f'Completed "{task.title}" — +{task.reward} credits')
    return redirect('tasks')


@login_required(login_url='login')
def fail_task(request, pk):
    task = Task.objects.get(id=pk)
    if request.user != task.owner:
        return HttpResponse('Not allowed')
    task.mark_failed()
    messages.warning(request, f'Failed "{task.title}" — -{task.penalty} credits')
    return redirect('tasks')
